...
class RFDN(nn.Module):
    def __init__(self, in_nc=3, nf=18, num_modules=6, out_nc=3, upscale=2, genotypes=None):
    ...
    def forward(self, input):
        ...
        in_B3 = self.c3(torch.cat([out_B1, out_B2], dim=1))
        out_B3 = self.B3(in_B3)

        in_B4 = self.c4(torch.cat([out_B2, out_B3], dim=1))
        out_B4 = self.B4(in_B4)

        in_B5 = self.c5(torch.cat([out_B2,out_B4], dim=1))
        out_B5 = self.B5(in_B5)

        in_B6 = self.c6(torch.cat([out_B4, out_B5], dim=1))
